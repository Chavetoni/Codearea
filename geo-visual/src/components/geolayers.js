...
// --- Styles ---
// Styles are assumed to be the same as in the previous version.
// For brevity, the existing 'styles' object is not repeated here.
// Ensure you have the 'styles' object from your original code.
const styles = {
  container: {
    width: '100%',
    height: '100vh',
...
const SoilLayersModel = () => {
  ...
  const [isHoveringButton, setIsHoveringButton] = useState({
      edit: false, add: false, remove: false, loadMap: false, removeMap: false
  });
  ...
  const BASE_DIMENSION_FOR_SCALING = 5; // Reference dimension for calculating strataScaleFactor
  const GROUND_GRID_SIZE = 100;
  const GROUND_GRID_DIVISIONS = 50;

  // Base sizes/scales for UI elements (will be multiplied by strataScaleFactor)
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (groundGridRef.current) {
        if(sceneRef.current && sceneRef.current.children.includes(groundGridRef.current)) sceneRef.current.remove(groundGridRef.current);
        disposeObject(groundGridRef.current);
        groundGridRef.current = null;
      }
      if (siteMapRef.current) {
        if(sceneRef.current && sceneRef.current.children.includes(siteMapRef.current)) sceneRef.current.remove(siteMapRef.current);
        disposeObject(siteMapRef.current);
        siteMapRef.current = null;
      }
      if (soilCubeRef.current) {
        if(sceneRef.current && sceneRef.current.children.includes(soilCubeRef.current)) sceneRef.current.remove(soilCubeRef.current);
        disposeObject(soilCubeRef.current);
        soilCubeRef.current = null;
      }
      if (sceneRef.current) {
          sceneRef.current.remove(ambientLight, directionalLight, backLight);
        ...
        siteMapRef.current.traverse((object) => {
            if (object.isMesh) {
                ...
                materials.forEach(material => {
                    if (material) {
                        material.transparent = siteMapProps.opacity < 1.0;
                        material.opacity = siteMapProps.opacity;
  ...
  const handleFileChange = (event) => {
    ...
    reader.onload = (e) => {
        if (!e.target || !e.target.result) {
            console.error("FileReader onload event error: No result.");
        ...
        try {
            loader.parse(
                ...
                (gltf) => { 
                    if (!gltf || !gltf.scene) {
                        console.error("GLTF parsing succeeded but returned invalid structure:", gltf);
                        alert("Failed to load map: Invalid GLTF structure.");
                        setMapFileName('');
                        if (fileInputRef.current) fileInputRef.current.value = null; 
                        return;
                    }

                    if (siteMapRef.current && sceneRef.current && sceneRef.current.children.includes(siteMapRef.current)) {
                        sceneRef.current.remove(siteMapRef.current);
                        siteMapRef.current.traverse(object => {
                            if (object.geometry) object.geometry.dispose();
                            if (object.material) {
                                if (Array.isArray(object.material)) object.material.forEach(m => { if(m.dispose) m.dispose();});
                    ...
                    let scaleFactor = 1.0;
                    let initialMapPosition = { x: 0, y: 0, z: 0 }; 
                    
                    try {
                        modelScene.updateMatrixWorld(true); 
                        const originalBox = new THREE.Box3().setFromObject(modelScene); 
                        const isValidBox = !originalBox.isEmpty() && 
                                      originalBox.max instanceof THREE.Vector3 && 
                                      originalBox.min instanceof THREE.Vector3 &&
                                      Number.isFinite(originalBox.max.x) && Number.isFinite(originalBox.max.y) && Number.isFinite(originalBox.max.z) &&
                                      Number.isFinite(originalBox.min.x) && Number.isFinite(originalBox.min.y) && Number.isFinite(originalBox.min.z);

                        if (!isValidBox) {
                             console.warn("Loaded model has an invalid or empty bounding box.");
                        } else {
                            const originalCenter = originalBox.getCenter(new THREE.Vector3());
                            modelScene.position.sub(originalCenter); 
                            modelScene.updateMatrixWorld(true); 
                            const centeredBox = new THREE.Box3().setFromObject(modelScene); 
                            const modelSize = centeredBox.getSize(new THREE.Vector3());

                            if (strataDimensions.width > 0 && (modelSize.x > 0.001 || modelSize.z > 0.001)) {
                                const largerModelDim = Math.max(modelSize.x, modelSize.z);
                                if (largerModelDim > 0) scaleFactor = strataDimensions.width / largerModelDim;
                            }
                            modelScene.scale.set(scaleFactor, scaleFactor, scaleFactor);
                            modelScene.updateMatrixWorld(true); 
                            const scaledMapBox = new THREE.Box3().setFromObject(modelScene); 
                            const isScaledBoxValid = !scaledMapBox.isEmpty() && 
                                                     scaledMapBox.max instanceof THREE.Vector3 && 
                                                     scaledMapBox.min instanceof THREE.Vector3 &&
                                                     Number.isFinite(scaledMapBox.max.y) && 
                                                     Number.isFinite(scaledMapBox.min.y);
                            if (!isScaledBoxValid) {
                                initialMapPosition.y = 0;
                            } else {
                                initialMapPosition.y = -scaledMapBox.min.y; 
                            }
                        }
                    } catch (calcError) {
                         console.error("Error during map centering/scaling/positioning:", calcError);
                         initialMapPosition.y = 0; 
                    }
                    modelScene.position.set(initialMapPosition.x, initialMapPosition.y, initialMapPosition.z); 
                    siteMapRef.current = modelScene;
                    sceneRef.current.add(siteMapRef.current);
                    setSiteMapProps({ 
                        position: { ...initialMapPosition }, 
                        scale: { x: scaleFactor, y: scaleFactor, z: scaleFactor },
                        visible: true, opacity: 1.0,
                    });
                    try { // Camera Adjustment
                         if (cameraRef.current && controlsRef.current) {
                            const combinedGroup = new THREE.Group();
                            if (soilCubeRef.current) combinedGroup.add(soilCubeRef.current.clone()); 
                            if (siteMapRef.current) combinedGroup.add(siteMapRef.current.clone());
                            if (combinedGroup.children.length > 0) {
                                const combinedBox = new THREE.Box3().setFromObject(combinedGroup);
                                if (!combinedBox.isEmpty() && Number.isFinite(combinedBox.max.x)) { // Basic check
                                    ...
                                    if (maxDim > 0) {
                                        ...
                                        cameraDist = Math.max(cameraDist, 10) * 1.75; 
                                        cameraRef.current.position.set( combinedCenter.x + cameraDist * 0.7, combinedCenter.y + cameraDist * 0.5, combinedCenter.z + cameraDist );
                ...
                (error) => { 
                    console.error('Error parsing GLB/GLTF model:', error);
                    alert(`Failed to parse the 3D map: ${error.message || 'Unknown error'}.`);
                    setMapFileName(''); if (fileInputRef.current) fileInputRef.current.value = null; 
                }
            );
        } catch (error) { 
            console.error('Error setting up GLTF loader or parsing:', error);
  ...
  const handleRemoveMap = () => {
    if (siteMapRef.current && sceneRef.current) {
        if (sceneRef.current.children.includes(siteMapRef.current)) {
            sceneRef.current.remove(siteMapRef.current);
        }
        siteMapRef.current.traverse(object => {
            if (object.geometry) object.geometry.dispose();
  ...
  const createTextLabel = useCallback((text, colorHexInput, strataScaleFactor = 1) => {
    ...
    if (typeof colorHex !== 'string' || !colorHex.startsWith('#')) colorHex = '#CCCCCC'; 
    ...
    sprite.scale.set(canvas.width * BASE_SPRITE_SCALE_FACTOR, canvas.height * BASE_SPRITE_SCALE_FACTOR, 1);
    return sprite;
  ...
  const createMarker = useCallback((number, strataScaleFactor = 1) => {
    ...
    const labelGeometry = new THREE.CircleGeometry(scaledRadius, segments); 
  ...
  const createDepthMarkers = useCallback((group, adjustedModelHeight, totalActualDepth, layerData, currentCubeWidth, strataScaleFactor = 1) => {
    ...
    if (layerData.length === 0 && adjustedModelHeight > 0 && depthPositions.length === 1) {
        ...
        const lastPushedY = depthPositions[depthPositions.length-1].yPosition;
        if (Math.abs(lastPushedY - (-adjustedModelHeight)) > 0.01 ) {
             depthPositions.push({ depth: parseFloat(totalActualDepth.toFixed(1)), yPosition: -adjustedModelHeight });
        }
    }
    ...
    depthPositions.forEach(item => {
      ...
      group.add(new THREE.Line(tickGeometry, markerMaterial));
      ...
      texture.needsUpdate = true;
      const labelMaterial = new THREE.MeshBasicMaterial({ map: texture, side: THREE.DoubleSide, transparent: true, depthTest: false });
      const labelPlaneGeometry = new THREE.PlaneGeometry(canvas.width * BASE_DEPTH_LABEL_PLANE_SCALE, canvas.height * BASE_DEPTH_LABEL_PLANE_SCALE); 
  ...
  const createSoilCube = useCallback((scene, layersCount, currentLayerData, currentStrataDimensions) => {
    ...
    if (soilCubeRef.current && scene.children.includes(soilCubeRef.current)) {
        soilCubeRef.current.traverse(object => { 
            if (object.geometry) object.geometry.dispose();
            if (object.material) {
                if (Array.isArray(object.material)) {
                    object.material.forEach(material => {
    ...
    newSoilCube.visible = showStrata; 
    ...
    if (showDepthMarkers && layersCount > 0) {
      createDepthMarkers(newSoilCube, adjustedModelHeight, totalActualDepth, currentLayerData, currentStrataDimensions.width, strataScaleFactor);
    }
    ...
    for (let i = 0; i < layersCount; i++) {
      const currentLayerInfo = currentLayerData[i] || {}; 
      const layerColorStr = currentLayerInfo.color || defaultSoilTypes[i % defaultSoilTypes.length].color;
      let colorHex = layerColorStr;
      try { new THREE.Color(colorHex); } catch (e) { colorHex = defaultSoilTypes[i % defaultSoilTypes.length].color; }
      
      const layerColor = new THREE.Color(colorHex); 
      const layerActualDepth = parseFloat(currentLayerInfo.depth) || 1; 
      ...
      if (viewMode === 'exploded' && layersCount > 1) {
          layerPositionsForConnectors.push({ top: currentYPosition, bottom: currentYPosition - layerVisualHeight });
      }

      const strataEdgeX = currentStrataDimensions.width / 2;
      const strataEdgeZ = currentStrataDimensions.depth / 2;
      ...
      if (showMarkers) {
        ...
        marker.position.set(strataEdgeX + markerGapFromEdge, layerCenterY, strataEdgeZ + elementZOffset); 
        layerGroup.add(marker);
      }

      if (showLabels) {
        // Construct the full label text including name and description
        const labelText = `${currentLayerInfo.name || 'Layer ' + (i + 1)}: ${currentLayerInfo.description || 'No description'}`;
        // Pass the full labelText to createTextLabel
        const labelSprite = createTextLabel(labelText, colorHex, strataScaleFactor); // FIXED: Pass full labelText
        ...
        if (showMarkers) {
            const markerRadiusWorld = BASE_MARKER_RADIUS * strataScaleFactor;
            const markerGapFromEdge = 0.4 * strataScaleFactor;
            ...
            const lineToMarkerGeo = new THREE.BufferGeometry().setFromPoints([
            ...
            layerGroup.add(new THREE.Line(lineToMarkerGeo, new THREE.LineBasicMaterial({color: 0xffffff})));

            labelAttachX = markerRightEdgeX + labelGapFromMarkerOrEdge; 
             const lineFromMarkerToLabelGeo = new THREE.BufferGeometry().setFromPoints([
            ...
            layerGroup.add(new THREE.Line(lineFromMarkerToLabelGeo, new THREE.LineBasicMaterial({color: 0xffffff})));
        } else {
            ...
            layerGroup.add(new THREE.Line(lineToLabelGeo, new THREE.LineBasicMaterial({color: 0xffffff})));
        }
        labelSprite.position.set(labelAttachX + labelSpriteWorldWidth / 2, layerCenterY, strataEdgeZ + elementZOffset);
        layerGroup.add(labelSprite);
      }
      newSoilCube.add(layerGroup); 
      currentYPosition = currentYPosition - layerVisualHeight;
      if (viewMode === 'exploded' && i < layersCount - 1 && layersCount > 1) {
    ...
    if (viewMode === 'exploded' && showConnectors && layerPositionsForConnectors.length > 1) {
      for (let i = 0; i < layerPositionsForConnectors.length - 1; i++) {
        const upperLayer = layerPositionsForConnectors[i]; 
        const lowerLayer = layerPositionsForConnectors[i+1];
        ...
        const connectorZ = currentStrataDimensions.depth / 2; 
        
        const rightLineGeometry = new THREE.BufferGeometry().setFromPoints([
          new THREE.Vector3(currentStrataDimensions.width / 2, connectorStartY, connectorZ), 
          new THREE.Vector3(currentStrataDimensions.width / 2, connectorEndY, connectorZ)
        ]);
        newSoilCube.add(new THREE.Line(rightLineGeometry, lineMaterial));
        const leftLineGeometry = new THREE.BufferGeometry().setFromPoints([
          new THREE.Vector3(-currentStrataDimensions.width / 2, connectorStartY, connectorZ),
          new THREE.Vector3(-currentStrataDimensions.width / 2, connectorEndY, connectorZ)
        ]);
        newSoilCube.add(new THREE.Line(leftLineGeometry, lineMaterial));
      }
    }
    soilCubeRef.current = newSoilCube;
  ...
  const createGroundGrid = useCallback((scene) => {
    ...
    if (groundGridRef.current && scene.children.includes(groundGridRef.current)) {
        ...
        if (Array.isArray(groundGridRef.current.material)) groundGridRef.current.material.forEach(m => m.dispose());
        else groundGridRef.current.material.dispose();
        scene.remove(groundGridRef.current);
        groundGridRef.current = null;
    }
    if (showGroundGrid) {
        const gridHelper = new THREE.GridHelper(GROUND_GRID_SIZE, GROUND_GRID_DIVISIONS, 0x888888, 0xbbbbbb);
        gridHelper.position.y = 0; 
        groundGridRef.current = gridHelper;
        scene.add(groundGridRef.current);
    }
  }, [showGroundGrid, GROUND_GRID_SIZE, GROUND_GRID_DIVISIONS]);

  const handleLayerChange = useCallback((e) => {
    const value = parseInt(e.target.value);
    if (!isNaN(value)) setNumLayers(Math.max(0, Math.min(value, 20))); 
  }, []);
    ...
    setLayerInfo(currentLayerInfo => {
      ...
      if (updatedLayerInfo[activeLayerIndex]) {
          if (property === 'depth') {
              ...
              updatedLayerInfo[activeLayerIndex] = { ...updatedLayerInfo[activeLayerIndex], [property]: isNaN(numericValue) || numericValue < 0.1 ? 0.1 : numericValue };
          } else {
  ...
  const handleAddLayer = useCallback(() => {
    if (numLayers === 0) {
        ...
        setNumLayers(prevNum => {
            const newNum = Math.min(prevNum + 1, 20);
            setActiveLayerIndex(newNum - 1); 
            return newNum;
        });
    }
  }, [numLayers]); 
  ...
  const handleRemoveLayer = useCallback(() => {
    ...
    setNumLayers(prevNum => {
        const newNumLayers = prevNum - 1;
        setActiveLayerIndex(prevIndex => Math.min(prevIndex, newNumLayers - 1 < 0 ? 0 : newNumLayers - 1));
        return newNumLayers;
    });
  }, [numLayers, activeLayerIndex]);
  
  const handleStrataDimensionChange = (axis, value) => {
    const numValue = parseFloat(value);
    if (isNaN(numValue) || numValue <= 0) return; 
    setStrataDimensions(prev => ({ ...prev, [axis]: numValue }));
  };
  ...
  useEffect(() => {
    if (sceneRef.current && createSoilCube && createGroundGrid && strataDimensions) { 
        ...
        if (numLayers > 0 && layerInfo.length > 0) {
            ...
            sceneRef.current.remove(soilCubeRef.current);
            soilCubeRef.current = null;
        }
    }
  }, [ numLayers, layerInfo, showLabels, showMarkers, showDepthMarkers, viewMode, verticalSpacing, showConnectors, createSoilCube, strataDimensions, showStrata, showGroundGrid, createGroundGrid ]); 
      ...
      <div style={styles.header}>
        <div style={styles.headerContent}>
          <div style={styles.controls}>
            <label style={styles.label} htmlFor="layers-input-main">Soil Layers:</label>
            <input id="layers-input-main" type="number" min="0" max="20" value={numLayers} onChange={handleLayerChange} style={styles.input} />
            <button onClick={() => setShowLayerPanel(!showLayerPanel)} style={{ ...styles.button, ...(isHoveringButton.edit ? styles.buttonHover : {}) }} onMouseEnter={() => setIsHoveringButton(prev => ({...prev, edit: true}))} onMouseLeave={() => setIsHoveringButton(prev => ({...prev, edit: false}))} >
        ...
        {showLayerPanel && (
          <div style={styles.editorPanel}>
            <CollapsibleSection title="Site Map (on Ground)" sectionKey="siteMap" isOpen={openSections.siteMap} onToggle={toggleSection}>
                ...
                <label htmlFor="file-upload-input-editor" style={{ ...styles.fileInputLabel, ...(isHoveringButton.loadMap ? styles.fileInputLabelHover : {})}} onMouseEnter={() => setIsHoveringButton(prev => ({...prev, loadMap: true}))} onMouseLeave={() => setIsHoveringButton(prev => ({...prev, loadMap: false}))} onClick={() => fileInputRef.current && fileInputRef.current.click()} role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') fileInputRef.current && fileInputRef.current.click();}} >
                ...
                {siteMapRef.current && ( 
                    <>
                        <label style={styles.mapControlLabel}>Position (X, Y above ground, Z):</label>
                        <div style={styles.mapControlContainer}>
                            {['x', 'y', 'z'].map((axis, index) => ( <input aria-label={`Map position ${axis.toUpperCase()}`} key={`pos-${axis}`} type="number" step="0.1" style={{...styles.mapControlInput, ...(index === 2 ? {marginRight: 0} : {})}} value={siteMapProps.position[axis]} onChange={(e) => handleMapPropChange('position', axis, e.target.value)} placeholder={axis.toUpperCase()} /> ))}
                        </div>
                        <label style={styles.mapControlLabel}>Scale (X, Y, Z):</label>
                         <div style={styles.mapControlContainer}>
                            {['x', 'y', 'z'].map((axis, index) => ( <input aria-label={`Map scale ${axis.toUpperCase()}`} key={`scale-${axis}`} type="number" step="0.1" min="0.01" style={{...styles.mapControlInput, ...(index === 2 ? {marginRight: 0} : {})}} value={siteMapProps.scale[axis]} onChange={(e) => handleMapPropChange('scale', axis, e.target.value)} placeholder={axis.toUpperCase()} /> ))}
                        </div>
                         <label htmlFor="opacity-slider-editor" style={styles.mapControlLabel}>Opacity:</label>
                         <input id="opacity-slider-editor" type="range" min="0" max="1" step="0.01" style={{width: '100%', marginBottom: '10px', cursor: 'pointer'}} value={siteMapProps.opacity} onChange={(e) => handleMapPropChange('opacity', null, e.target.value)} aria-label="Map opacity slider" />
                        <label style={styles.checkboxLabel}> <input type="checkbox" checked={siteMapProps.visible} onChange={(e) => handleMapPropChange('visible', null, e.target.checked)} style={styles.checkbox} aria-label="Show site map checkbox" /> Show Site Map </label>
                        <button onClick={handleRemoveMap} style={{ ...styles.actionButton, ...styles.removeButton, width: '100%', marginTop: '10px', ...(isHoveringButton.removeMap ? styles.removeButtonHover : {})}} onMouseEnter={() => setIsHoveringButton(prev => ({...prev, removeMap: true}))} onMouseLeave={() => setIsHoveringButton(prev => ({...prev, removeMap: false}))} > Remove Site Map </button>
                    </>
                )}
            </CollapsibleSection>
            ...
            <CollapsibleSection title="Display Options" sectionKey="displayOptions" isOpen={openSections.displayOptions} onToggle={toggleSection}>
                <div style={styles.checkboxContainer}>
                    <label style={styles.checkboxLabel}> <input type="checkbox" checked={showGroundGrid} onChange={(e) => setShowGroundGrid(e.target.checked)} style={styles.checkbox} aria-label="Show ground grid checkbox" /> Show Ground Grid </label>
                    <label style={styles.checkboxLabel}> <input type="checkbox" checked={showStrata} onChange={(e) => setShowStrata(e.target.checked)} style={styles.checkbox} aria-label="Show strata model checkbox" disabled={numLayers === 0} title={numLayers === 0 ? "Initialize layers first" : ""} /> Show Strata Model </label>
                    <label style={styles.checkboxLabel}> <input type="checkbox" checked={showLabels} onChange={(e) => setShowLabels(e.target.checked)} style={styles.checkbox} aria-label="Show layer labels checkbox" disabled={numLayers === 0}/> Show Layer Labels </label>
                    <label style={styles.checkboxLabel}> <input type="checkbox" checked={showMarkers} onChange={(e) => setShowMarkers(e.target.checked)} style={styles.checkbox} aria-label="Show layer markers checkbox" disabled={numLayers === 0}/> Show Layer Markers </label>
                    <label style={styles.checkboxLabel}> <input type="checkbox" checked={showDepthMarkers} onChange={(e) => setShowDepthMarkers(e.target.checked)} style={styles.checkbox} aria-label="Show depth scale checkbox" disabled={numLayers === 0}/> Show Depth Scale </label>
                </div>
            </CollapsibleSection>
